...
export function validateAnswers(questions: any[], answers: Map<number, number[]>): {
	isValid: boolean;
	errors: string[];
	warnings: string[];
} {
	...
	if (unansweredCount > 0) {
		warnings.push(`Tienes ${unansweredCount} pregunta${unansweredCount > 1 ? 's' : ''} sin responder`);
	}
